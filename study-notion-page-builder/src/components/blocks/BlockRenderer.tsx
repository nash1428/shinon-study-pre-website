...
export const BlockRenderer: React.FC<Props> = ({ block, onChange, onAddBlock }) => {
  switch (block.type) {
    case "heading":
    ...
    case "toggle":
      return <ToggleBlock block={block} onChange={onChange} childrenIds={block.children?.map((c) => c.id) || []} />;
    case "callout":
